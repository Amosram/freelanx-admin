...
const Login = ({setToken}) => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
             e.preventDefault();
             const response = await axios.post("https://freelanx-server.onrender.com/api/user/admin", {email, password});
    ...
    <div className='min-h-screen flex items-center justify-center w-full'>
        <div className='bg-white shadow-md rounded-lg px-8 py-6 max-w-md'>
            ...
            <form onSubmit={onSubmitHandler}>
                ...
                <button type='submit' className='hover:cursor-pointer rounded-md mt-2 bg-grad text-gray-50 px-3 py-2 w-full'>Login</button>
            </form>
        </div>
    </div>
  )
